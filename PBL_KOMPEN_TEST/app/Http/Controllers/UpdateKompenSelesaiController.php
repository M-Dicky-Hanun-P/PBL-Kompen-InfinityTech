    ...
    public function index()
{
    ...
    foreach ($tugasKompen as $tugas) { //fetch data di m_tugas_kompen
        $pemberiTugas = null;
        $nama_tugas = '';
        $jamKompen = '';
        $waktuPengerjaan = '';
    ...
    public function upload(Request $request, $id_riwayat)
{
    ...
    try {
        // Find the record by id_riwayat (not id)
        $riwayatKompen = RiwayatKompenModel::findOrFail($id_riwayat);
        ...
        return redirect()->route('mUpdateKompenSelesai.index')->with('success', 'File berhasil diunggah');
    } catch (\Exception $e) {
